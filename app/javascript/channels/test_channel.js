...
consumer.subscriptions.create({channel: "TestChannel"},{
  connected() {
  ...
  received(data) {
    ...
    if(data["map_id"] == map_id ) {
      ...
      if(data["user_id"] != current_user) {
        ...
        switch(data["action"]) {
          case "user_left":
            ...
            $('*[data-student="filter_' + data["user_id"] + '"]').attr('data-student', "filter_left");
          ...
          case "user_joined":
            ...
            let new_user = '<div id="student_'+ new_user_id +'" style="display: inline-block; margin: 0.5em; border-bottom: 2px solid ' + new_user_color + '"><label> ' + new_user_name + ' <input type="checkbox" class="filter_students" id="filter_'+ new_user_id +'" checked/></label></div>'
          ...
          case "create":
            ...
            case "update":
              if(data["type"] == "node") {
                ...
                updateNode(data["id"], data["label"], data["x"], data["y"], data["color"], data["lock"], data["user_color"])
              } else if(data["type"] == "link"){
                // Display the updated link
                updateEdge(data["id"], data["start"], data["end"], data["label"], data["lock"], data["user_color"])
              }
              break;
              ...
              case "destroy":
                if(data["type"] == "node") {
                  ...
                  if($("#edit-dialog").is(":visible") && ($("#start").val() == data["id"] || $("#end").val() == data["id"])) {
      ...
      if (data["action"] == 'user_left') {
          ...
          let html = ''
          if(language == "de") {
            html = '<div id="note" style="position: absolute;right: 50%; z-index: 1; display: inline-block;  -webkit-transform: translate(50%, 50%); transform: translate(50%, 50%); background-color: #1b809e; padding: 2em; color: white;">Du hast ComapEd in einem anderen Fenster geöffnet. Du kannst dieses Fenster nun schließen! </div>'
          }else{
            html = '<div id="note" style="position: absolute;right: 50%; z-index: 1; display: inline-block;  -webkit-transform: translate(50%, 50%); transform: translate(50%, 50%); background-color: #1b809e; padding: 2em; color: white;">You opened ComapEd in another browser tab. Close this tab, please</div>'
          }
      ...
      case "create":
        if(data["type"] == "node") {
          if(language == "de") {
            ...
            change += "<li>die Farbe ist <span style='color: "+data["color"]+"'>&#9632;</span> (" + data["color"] + ")</li>"
            change += "<li>die Position lautet [x: " + Math.round(parseInt(data["x"])) + "; y: " + Math.round(parseInt(data["y"])) + "]</li>"

          }else{
            object = 'CREATED NODE'
            change += "<li>created" + " the node " + data["label"] + "</li>"
            change += "<li>the color is <span style='color: "+data["color"]+"'>&#9632;</span> (" + data["color"] + ")</li>"
            change += "<li>the position is [x: " + Math.round(parseInt(data["x"])) + "; y: " + Math.round(parseInt(data["y"])) + "]</li>"
          }
        ...
        case "update":
          if(data["type"] == "node") {
            ...
            if(data["color_old"] != data["color"]){
              if(language == "de") {
                change += "<li>änderte die Farbe zu <span style='color: "+data["color"]+"'>&#9632;</span> (" + data["color"] + ")</li>"

              }else{
                change += "<li>set the color to <span style='color: "+data["color"]+"'>&#9632;</span> (" + data["color"] + ")</li>"
              }

            }
            if(data["x_old"] != data["x"] || data["y_old"] != data["y"]){
              if(language == "de") {
                change += "<li>änderte die Position zu [x: " + Math.round(parseInt(data["x"])) + "; y: " + Math.round(parseInt(data["y"])) + "]</li>"
              }else{
                change += "<li>set the position to [x: " + Math.round(parseInt(data["x"])) + "; y: " + Math.round(parseInt(data["y"])) + "]</li>"
              }
    ...
    if($('#filter_'+data["user_id"]).is(":checked")){
      // Show the log
      log = '<div data-student="filter_'+data["user_id"]+'" class="log"> <div id="log_header"> <div class="change_header"> ' + object + ' </div> </div> <div id="node_log" class="log_entry"> <div class="student" style="border-left: 12px solid ' + data["user_color"]+' "> <span class="student_name">' + user + '</span> </div> <div class="context"> <ul class="changes"> ' + change + ' </ul> </div> </div> </div>'

    }else{
      // Hide the log
      log = '<div style="display: none;" data-student="filter_'+data["user_id"]+'" class="log"> <div id="log_header"> <div class="change_header"> ' + object + ' </div> </div> <div id="node_log" class="log_entry"> <div class="student" style="border-left: 12px solid ' + data["user_color"]+' "> <span class="student_name">' + user + '</span> </div> <div class="context"> <ul class="changes"> ' + change + ' </ul> </div> </div> </div>'
