class ConceptsController < ApplicationController
  ...
  def create
    ...
    ActionCable.server.broadcast("test_channel", {action: "create", type: "node",
      user: @current_student.name, user_id: @current_student.id, user_color: @current_student.color,
      label: @concept.label, id: @concept.id, x: @concept.x, y: @concept.y, color: @concept.color, map_id: @map.id})
  end
  ...
  def update
    ...
    if @concept.update(concept_params)
      unless concept_params[:label] == old_label
        @map.versionize(DateTime.now)
      end
    end
    render :create
    ...
    ActionCable.server.broadcast("test_channel", {action: "update", type: "node",
    user: @current_student.name, user_id: @current_student.id, user_color: @current_student.color,
    label: @concept.label, id: @concept.id, x: @concept.x, y: @concept.y, color: @concept.color,
    lock: @concept.lock, label_old: old_label, x_old: old_x, y_old: old_y, color_old: old_color, lock_old: old_lock,
    map_id: @map.id})
  end
  ...
  def destroy
    ...
    ActionCable.server.broadcast("test_channel", {action: "destroy", type: "node",
    user: @current_student.name, user_id: @current_student.id, user_color: @current_student.color,
    label: @concept.label, id: @concept.id, x: @concept.x, y: @concept.y, color: @concept.color, map_id: @map.id})
  end
  ...
  def concept_params
    ...
    params.require(:concept).permit([:label, :lock, :x, :y, :color])
  end
